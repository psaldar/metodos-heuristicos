...
def update_poblacion(X_train, Y_train, X_val, Y_val, poblacion_prev, fitness, hijos_prev, k, bin_cont = False):
    ...
    ### Agrego tanto hijos como individuos de la poblacion a una sola lista
    res = []
    for ind in poblacion:
        res.append([ind['fitness'].copy(),ind.copy()])
    
    for hijo in hijos:
        res.append([hijo['fitness'].copy(),hijo.copy()])
    
    ### organizo los individuos por su fitness
    res.sort(reverse = True, key=myFunc)
    
    N2 = len(res)
    ### inicializo variables para comenzar a actualizar los individuos de la 
    ### poblacion buscando que los individuos sean lo mas diferente posible
    ids_check = []
    new_population = []
    
    ### Agrego a la nueva poblacion el mejor individuo
    ids_check.append(0)
    new_population.append(res[0][1].copy())
    
    ### Calculo la matriz de distancias de los individuos de la poblacion
    mat_dist = np.zeros(shape = (N2,N2))
    ...
    for i in range(N2):
        for j in range(N2):
            ...
            sol1_arr = funciones.sol2array(sol1, bin_cont = bin_cont)
            sol2_arr = funciones.sol2array(sol2, bin_cont = bin_cont)
            
            sol1_arr_z = np.array(funciones.estandatizar_sol(sol1_arr))
            sol2_arr_z = np.array(funciones.estandatizar_sol(sol2_arr))
            
            mat_dist[i,j] = np.linalg.norm(sol1_arr_z-sol2_arr_z)
    
    ### comenzamos a actualizar la poblacion con base en la matriz de distancias
    for _ in range(1,N):
        ...
        for index in ids_check:
            mas_cercanos = np.argsort(mat_dist[index,:])[1:]
            
            for v in range(len(mas_cercanos)):
                if mas_cercanos[v] not in ids_check:
                    dic_cercanos[mas_cercanos[v]] = dic_cercanos[mas_cercanos[v]] + v
                    
        max_sum = -1
        a_agregar = 0
        for key in dic_cercanos.keys():
            if max_sum < dic_cercanos[key]:
                max_sum = dic_cercanos[key]
                a_agregar = key
        
        ids_check.append(a_agregar)
        new_population.append(res[a_agregar][1].copy())
    
    return new_population, fitness, new_population[0].copy(), new_population[0]['fitness'].copy()
...
def busqueda_exahustiva(X_train, Y_train, X_val, Y_val, k, best_mod):
    ...
    for f_activation in ["identity", "logistic"]:
    ...
    for i in tqdm(range(num_generacion)):
        ...
        for j in range(population_size - cant_seleccion):
            ...
            if random.random() < prob_mutacion:
                ### solamente se va a mutar un hijo si es malo, de lo contrario
                ### no se muta
                if hijo['fitness']< threshold_roc:
                    logger.info(f'MUTA HIJO NO TAN BUENO')
                    hijo = mutacion(hijo,
                                    min_lay,
                                    max_lay,
                                    min_neu,
